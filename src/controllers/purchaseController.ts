...
export const createPurchase = async (req: Request, res: Response) => {
  try {
    ...
    const { vendor_id, date, payment_method, payment_account_id, notes, items } = req.body;

    if (!vendor_id || !date || !payment_method || !items) {
      return res.status(400).json(formatError('vendor_id, date, payment_method, and items are required'));
    }
...
export const getPurchaseById = async (req: Request, res: Response) => {
  try {
    ...
    const purchase = await purchaseService.getPurchaseById(outletId, req.params.id as string);
    if (!purchase) return res.status(404).json(formatError('Purchase Order not found'));
...
export const receiveGoods = async (req: Request, res: Response) => {
  try {
    ...
    if (!items || !Array.isArray(items) || items.length === 0) {
      return res.status(400).json(formatError('items array with product_id and quantity_received is required'));
    }

    const result = await purchaseService.receiveGoods(outletId, req.params.id as string, items, notes);
...
export const deletePurchase = async (req: Request, res: Response) => {
  try {
    ...
    return res.json(formatSuccess({ message: 'Purchase Order deleted successfully' }));
  } catch (error: any) {
