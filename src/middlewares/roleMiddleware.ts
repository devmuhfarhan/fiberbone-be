 ...
 */
export const authorizePermission = (moduleSlug: string, action: PermissionAction) => {
  return async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
    if (!req.user) {
      logger.warn('authorizePermission: req.user is undefined');
      res.status(401).json(formatError('Access denied. User not authenticated.'));
      return;
    }
    ...
    try {
      ...
      if (cached !== null) {
        ...
        logger.warn(`Permission denied (cache): role='${role}', module='${moduleSlug}', action='${action}'`);
        res.status(403).json(formatError('Access denied. Insufficient permissions.'));
        return;
      }

      // Cache miss — query database
      const allowed = await rolePermissionModel.checkPermission(role, moduleSlug, action);
      ...
      logger.warn(`Permission denied (db): role='${role}', module='${moduleSlug}', action='${action}'`);
      res.status(403).json(formatError('Access denied. Insufficient permissions.'));
    } catch (err) {
      logger.error('authorizePermission error', err);
      res.status(500).json(formatError('Internal server error during authorization.'));
    }
  };
};
